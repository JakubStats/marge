...
stat_out <- function(Y, B1, TSS, GCV.null, pen = c("two", "log"), ...) {
  pen <- match.arg(pen)
  N <- length(Y)
  ...
  if (sum(is.na(reg$coef)) == 0) {
    ...
    # ED: changed 'pen'
    df1a <- switch(pen,
                   two = ncol(B1) + 2*(ncol(B1) - 1)/2, # This matches the earth() package, SAS and Friedman (1991) penalty.
                   log = ncol(B1) + log((ncol(B1) - 1)/2) # This NEEDS CHECKING
    )
...
stat_out_score_null_OLD <- function(Y, N, n, id, family = "gaussian", corstr = "independence", B_null, nb = FALSE, is.gee = FALSE, ...) {
...
score_fun_gee_OLD <- function(Y, N, n_vec, VS.est_list, AWA.est_list, J2_list, Sigma2_list, J11.inv, JSigma11, mu.est, V.est, B1, XA, nb = FALSE, ...) {
  reg <- try(stats::lm.fit(B1, Y, ...), silent = TRUE)  # This is not the model fit!! It just checks whether any issues occur for a simple linear regression model.
...
backward_sel <- function(Y, B_new, pen = c("two", "log"), GCV.null = 0.001, ...) {
  pen <- match.arg(pen)
  N <- length(Y)
  ...
  for (j in 1:(ncol(B_new) - 1)) {
    ...
    p <- switch(pen,
                two = ncol(B_new1) + 2*(ncol(B_new1) - 1)/2,  # This matches the earth() package, SAS and Friedman (1991) penalty.)
                log = ncol(B_new1) + log(ncol(B_new1) - 1)/2  # This NEEDS CHECKING
    )
...
#' mod <- marge(X_pred, Y, N, n, id, family, corstr, pen, tols_score,
#'              M, minspan, print.disp, nb, is.gee)
marge_OLD <- function(X_pred, Y, N, n = 1, id = c(1:length(Y)), family = "gaussian", corstr = "independence", pen = 2, tols_score = 0.00001, M = 21, minspan = NULL, print.disp = FALSE, nb = FALSE, is.gee = FALSE, ...) {
